...
it("has a route handler listening to /api/tickets for poast request", async () => {
  const response = await await request(app).post("/api/tickets").send({});
  expect(response.status).not.toEqual(404);
});

it("it can only be accesed if the user is signed in", async () => {
...
it("it creates a ticket with valid inputs", async () => {
  // add in a check ticket was saved
